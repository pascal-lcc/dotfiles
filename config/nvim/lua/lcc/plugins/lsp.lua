        ...
        vim.api.nvim_create_autocmd("FileType", {
            ...
            callback = function(args)
                ...
                local expected_lsps = {
                    ...
                    "clangd", -- C/C++
                    "typescript-language-server", -- TS/JS 本體 (與設定檔中的 ts_ls 完美對接)
                    "vue-language-server", -- Vue 3 本體 (與設定檔中的 vue_ls 完美對接)
                    "prettierd", -- Web (JS/TS/JSON/HTML/CSS/Markdown)
                    "stylua", -- Lua
                    "black", -- Python
                }

                -- 🌟 3. 呼叫 Mason 註冊表 API，進行「全自動背景盲裝」
                local registry = require("mason-registry")

                -- 當 Mason 註冊表載入完成後（或確保它就緒），開始掃描
                registry.refresh(function()
                    for _, lsp_name in ipairs(expected_lsps) do
                        -- 檢查這個 LSP 有沒有被 Mason 實體安裝過
                        if not registry.is_installed(lsp_name) then
                            local p = registry.get_package(lsp_name)

                            -- 🌟 核心防禦：如果這個套件此時此刻「正在下載中」，就直接跳過，不重複觸發！
                ...
                -- 1. 先安全地把 blink 撈進來準備拿能力
                pcall(vim.cmd.packadd, "blink.cmp")
                local blink_ok, blink = pcall(require, "blink.cmp")
                local my_capabilities = nil

                if blink_ok then
                    -- 拿取 blink 專屬的能力規格
                    my_capabilities = blink.get_lsp_capabilities()
                end
                ...
                vim.diagnostic.config({
                    ...
                    update_in_insert = false, -- 🌟 重點：不要在打字時一直跳錯誤（這會導致 E 一直閃爍不見），離開輸入模式或儲存時再刷新，最穩定！
                    -- update_in_insert = true,
                    underline = true, -- 在寫錯的程式碼底下畫紅線
                    severity_sort = true,
                })
                ...
                -- 2. 直接塞進特定 LSP 的配置裡，完全不碰脆弱的全域 default_config
