...
function M.setup()
    ...
    vim.api.nvim_create_autocmd("ModeChanged", {
        ...
        callback = function()
            ...
            -- 只有在群組切換時才發送
            if last_sent_group ~= current_group then
                last_sent_group = current_group
                
                if current_group == STATUS_GROUP.EDIT then
                    -- send_to_term("\27]1337;SetUserVar=IS_NVIM=T05J\7") -- ONI
                else
                    -- send_to_term("\27]1337;SetUserVar=IS_NVIM=T04=\7")  -- ON
                end
            end
        end,
    })
    ...
    vim.api.nvim_create_autocmd({ "VimSuspend", "VimLeavePre"}, {
        group = wezterm_sync_nvim,
        callback = function()
            last_sent_status = nil
            send_to_term("IS_NVIM", "")
        end,
    })
...
vim.api.nvim_create_autocmd("FocusGained", {
    pattern = "*",
    callback = function()
        -- 如果你想除錯，用 notify 比較明顯
        send_to_term("pane_ctx", "NVIM")
        -- 在這裡發射你的 pane_ctx 給 WezTerm
        -- 確保即便在 Nvim 裡，WezTerm 也能收到正確的變數
        -- os.execute("send_to_term pane_ctx nvim SetUserVar")
    end,
})
